...
class Util(commands.Cog):
  ...
  async def help(self,ctx,*cog):
    ...
    if not cog:
       help=discord.Embed(title='Listing Commands', description='Hi , im `{}` made by `{}` . My prefixes are `c!`,`covid ` and `bot mention` . Here is a list of my commands :'.format(self.client.user.name,'Jake'),color=discord.Colour(value=16730698))
       cmds_desc = ''
