...
def get_country_infections(country:str=None):
	infections = {}
	labels = ['country','cases','new_cases','deaths','new_deaths','recovered','active','serious','total_per_million','deaths_per_million' , 'total_tests' , 'tests_per_million' , 'first_case']
	for tr in html.find('tbody').find_all('tr'):
		for (index,td) in enumerate(tr.find_all('td')):
			if (index == 0) :
				country_key = td.getText().strip()
